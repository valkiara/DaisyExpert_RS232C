...
namespace DERS232C
{
    ...
    */
    public class DEClass
    {
        ...
        public DEClass(string Name, StopBits StopBits = StopBits.One, int Baudrate = 9600, int DataBits = 8, Parity Parity = Parity.None, Handshake Handshake = Handshake.None)
        {
            ...
            SerialPort.Encoding = UnicodeEncoding.Unicode;
            SerialPort.DataReceived += port_OnReceiveDatazz;
        }
        ...
        /// <returns></returns>
        private string SendCommand(byte CMD, string Data = "", byte exSEQ = 0)
        {
            xCMD = CMD;
            ...
            _wait.WaitOne();
            Thread.Sleep(180);
            var x = Read(SerialPort, xCMD, xData);
            //Console.WriteLine(x);
            //Console.ReadLine();
            return x;
        }

        private string Read(SerialPort xPort, byte CMD, string xData)
            {

                int SYNCount = 0;
                Byte[] Baf = new Byte[xPort.ReadBufferSize];
                List<Byte> lstB = new List<byte>();

                int bytesToRead = 0;
                bytesToRead = xPort.Read(Baf, 0, Baf.Length);
                int readByte = Baf[0];
        ...
        private void port_OnReceiveDatazz(object sender, SerialDataReceivedEventArgs e)
        {
            ...
            //    SerialPort xPort = (SerialPort)sender;
            //    Byte[] Baf = new Byte[xPort.ReadBufferSize];
            //    List<Byte> lstB = new List<byte>();
